...
static void frontend_state_trade(GuiEvent event)
{
	...
	case GUI_UPDATE:
		...
		frontend_gui_check(GUI_TRADE_GIVE, can_give_all_resources());
  ...
  case GUI_TRADE_GIVE:
    trading = TRUE;
    quote = trade_top_quote();
    if (quote != NULL && quote->is_domestic) {
      ...
      trade_new_trade();
      cb_domestic(trade_give_supply(), trade_give_receive());
    }
    return;
