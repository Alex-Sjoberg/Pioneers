...
static gint give_supply_request[NO_RESOURCE];
static gint give_receive_request[NO_RESOURCE];

/** This button can be hidden in games without interplayer trade */
static GtkWidget *call_btn;
/** This button can be hidden in games without interplayer trade */
static GtkWidget *give_btn;
...
/** @return TRUE if at least one resource is available */
gboolean can_give_all_resources(void)
{
  gint idx;
  for (idx = 0; idx < NO_RESOURCE; idx++) {
    if (resource_asset(idx) > 0) return TRUE;
  }
  return FALSE;
}

/** @return the top quote */
const QuoteInfo *trade_top_quote(void)
{
  return quotelist_first(QUOTEVIEW(quoteview)->quote_list);
}
...
const gint *trade_give_supply(void)
{
  gint idx;
  for (idx = 0; idx < NO_RESOURCE; idx++) {
    give_supply_request[idx] = resource_asset(idx)>0;
  }
	return give_supply_request;
}

/** @return no resources */
const gint *trade_give_receive(void)
{
  gint idx;
  for (idx = 0; idx < NO_RESOURCE; idx++) {
    give_receive_request[idx] = 0;
  }
	return give_receive_request;
}
...
GtkWidget *trade_build_page(void)
{
	...
	give_box = gtk_hbutton_box_new();
	gtk_widget_show(give_box);
	gtk_box_pack_start(GTK_BOX(we_receive_frame), give_box, FALSE, TRUE,
			   0);

	give_btn = gtk_button_new_with_mnemonic(_("_Give All Resources"));
	frontend_gui_register(give_btn, GUI_TRADE_GIVE, "clicked");
	gtk_widget_show(give_btn);
	gtk_container_add(GTK_CONTAINER(give_box), give_btn);
